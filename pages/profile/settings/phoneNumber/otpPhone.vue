<template>
        ...
        <form @submit.prevent="changePhoneCheckCode">
          <div class="contain-otp flex-center mb-3">
            ...
            <v-otp-input ref="otpInput" input-classes="otp-input" separator="-" :num-inputs="4" :should-auto-focus="true"
              :is-input-num="true" :conditionalClass="['one', 'two', 'three', 'four']" :placeholder="['*', '*', '*', '*']"
  ...
  <Dialog v-model:visible="showSuccess" modal :style="{ width: '50rem' }"
    ...
    <div class="flex-center">
      <ui-base-button mode="main_btn" @click="visible = false" link :to="localePath('/profile/settings')"> {{
        $t("Settings") }} </ui-base-button>
    </div>
  </Dialog>
</template>
